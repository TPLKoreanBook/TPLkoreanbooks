...
for book in books:
    ...
    kbt_author = book['translated_author'].replace(' ', '')
    tlt = book['translated_title']

    if kbt != '':
        book['kyobo_title'] = kbt.split('|')[0].replace('(원서번역서', '').replace('( HardCover)', '').replace('-', '').replace(kbt_author, '').strip()
    if kbt.count('검색 ') != 0:
        book['kyobo_title'] = ''
    if abt != '':
